...
*/
func (a *DriversApiService) CreateDriver(ctx _context.Context) apiCreateDriverRequest {
	return apiCreateDriverRequest{
		apiService: a,
		ctx:        ctx,
	}
...
type apiGetDriverTachographActivityRequest struct {
	ctx        _context.Context
	apiService *DriversApiService
	startTime  *time.Time
	endTime    *time.Time
...
func (r apiGetDriverTachographActivityRequest) StartTime(startTime time.Time) apiGetDriverTachographActivityRequest {
	r.startTime = &startTime
	return r
}

func (r apiGetDriverTachographActivityRequest) EndTime(endTime time.Time) apiGetDriverTachographActivityRequest {
	r.endTime = &endTime
	return r
}

func (r apiGetDriverTachographActivityRequest) DriverIds(driverIds []string) apiGetDriverTachographActivityRequest {
	r.driverIds = &driverIds
	return r
}

func (r apiGetDriverTachographActivityRequest) After(after string) apiGetDriverTachographActivityRequest {
	r.after = &after
	return r
}

/*
GetDriverTachographActivity Get driver tachograph activity
Returns all known tachograph activity for all specified drivers in the time range.
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
@return apiGetDriverTachographActivityRequest
*/
func (a *DriversApiService) GetDriverTachographActivity(ctx _context.Context) apiGetDriverTachographActivityRequest {
	return apiGetDriverTachographActivityRequest{
		apiService: a,
		ctx:        ctx,
	}
...
*/
func (r apiGetDriverTachographActivityRequest) Execute() (DriverTachographActivityResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodGet
		localVarPostBody     interface{}
		localVarFormFileName string
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  DriverTachographActivityResponse
	)

	localBasePath, err := r.apiService.client.cfg.ServerURLWithContext(r.ctx, "DriversApiService.GetDriverTachographActivity")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/fleet/drivers/tachograph-activity/history"
	...
	if r.driverIds == nil {
		return localVarReturnValue, nil, reportError("driverIds is required and must be specified")
	}
	...
	if localVarHTTPResponse.StatusCode >= 300 {
		...
		if localVarHTTPResponse.StatusCode == 200 {
			var v DriverTachographActivityResponse
			err = r.apiService.client.decode(&v, localVarBody, localVarHTTPResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHTTPResponse, newErr
